...
if (isset($_POST['add'])) {
	...
	$query = "INSERT INTO newcandidate(nc_name,nc_age,nc_phak,nc_lat,nc_saonoum,nc_part,nc_reason,nc_pic)VALUES(?,?,?,?,?,?,?,?)";
	$stmt = $conn->prepare($query);
	$stmt->bind_param("ssssssss", $nc_name, $nc_age, $nc_phak, $nc_lat, $nc_saonoum, $nc_part, $nc_reason, $pic_rand);
	$stmt->execute();
...
if (isset($_POST['update'])) {
	...
	$query = "UPDATE newcandidate SET nc_name=?, nc_age=?, nc_phak=?, nc_lat=?, nc_saonoum=?, nc_part=?, nc_reason=?, nc_pic=? WHERE nc_id=?";
	$stmt = $conn->prepare($query);
	$stmt->bind_param("ssssssssi", $nc_name, $nc_age, $nc_phak, $nc_lat, $nc_saonoum, $nc_part, $nc_reason, $nnc_pic, $nc_id);
	$stmt->execute();
