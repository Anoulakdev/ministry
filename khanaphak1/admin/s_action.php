...
if (isset($_GET['deleteall'])) {
	...
	try {
		...
		// ลบ oscore1
		$stmt1 = $conn->prepare("DELETE FROM oscore1 WHERE s_no=?");
		$stmt1->bind_param("i", $s_no);
		$stmt1->execute();

		// ลบ oscore2
		$stmt2 = $conn->prepare("DELETE FROM oscore2 WHERE s_no=?");
		$stmt2->bind_param("i", $s_no);
		$stmt2->execute();

		// ลบ nscore1
		$stmt3 = $conn->prepare("DELETE FROM nscore1 WHERE s_no=?");
		$stmt3->bind_param("i", $s_no);
		$stmt3->execute();

		// ลบ nscore2
		$stmt4 = $conn->prepare("DELETE FROM nscore2 WHERE s_no=?");
		$stmt4->bind_param("i", $s_no);
		$stmt4->execute();

		// ถ้าทุกอันผ่าน → commit
		$conn->commit();

		header("refresh:1; url=deletesheet");
	} catch (Exception $e) {
		// มี error → rollback กลับ
		$conn->rollback();
