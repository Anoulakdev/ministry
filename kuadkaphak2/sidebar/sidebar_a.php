<aside id="sidebar" class="sidebar">

    <ul class="sidebar-nav" id="sidebar-nav">

        <li class="nav-item">
            <a class="nav-link collapsed" href="home">
                <i class="bi bi-grid"></i>
                <span>ໜ້າ​ຫຼັກ</span>
            </a>
        </li><!-- End Dashboard Nav -->


        <li class="nav-item">
            <a class="nav-link collapsed" href="admin">
                <i class="bi bi-person"></i>
                <span>ຜູ້​ດູ​ແລລະບົບ</span>
            </a>
        </li><!-- End Profile Page Nav -->

        <!-- <li class="nav-item">
            <a class="nav-link collapsed" href="oldcandidate">
                <i class="bi bi-person"></i>
                <span>ຜູ້​ສະ​ໝັກຊຸດ​ເກົ່າ</span>
            </a>
        </li> -->

        <li class="nav-item">
            <a class="nav-link collapsed" href="oldcandidate">
                <i class="bi bi-person"></i>
                <span>ຜູ້​ສະ​ໝັກ(ປະ​ທານ​ກວດກ​າ​ພັກ)</span>
            </a>
        </li>

        <li class="nav-item">
            <a class="nav-link collapsed" href="newcandidate">
                <i class="bi bi-person"></i>
                <span>ຜູ້​ສະ​ໝັກ(ຮອງປະ​ທານ​ກວດກ​າ​ພັກ)</span>
            </a>
        </li>

        <li class="nav-item">
            <a class="nav-link collapsed" href="member">
                <i class="bi bi-person"></i>
                <span>ກຳ​ມະ​ການ</span>
            </a>
        </li>

        <li class="nav-item">
            <a class="nav-link collapsed" href="sheet">
                <i class="bi bi-receipt"></i>
                <span>ໃບ​ລົງ​ຄະ​​ແນນ</span>
            </a>
        </li>

        <!-- <li class="nav-item">
            <a class="nav-link collapsed" href="membersearch">
                <i class="bi bi-receipt"></i>
                <span>ລາຍ​ລະ​ອຽດກຳ​ມະ​ການ​</span>
            </a>
        </li>

        <li class="nav-item">
            <a class="nav-link collapsed" href="sheetsearch">
                <i class="bi bi-receipt"></i>
                <span>ລາຍ​ລະ​ອຽດ​ໃບ​ລົງ​ຄະ​ແນນ</span>
            </a>
        </li>

        <li class="nav-item">
            <a class="nav-link collapsed" href="candidatedetail">
                <i class="bi bi-receipt"></i>
                <span>ລາຍ​ລະ​ອຽດ​ຜູ້​ສະ​ໝັກ</span>
            </a>
        </li> -->

        <li class="nav-item">
            <a class="nav-link collapsed" href="sheetsearch1">
                <i class="bi bi-receipt"></i>
                <span>ລາຍ​ລະ​ອຽດລົງ​ຄະ​ແນນ(ປະ​ທານ)</span>
            </a>
        </li>

        <li class="nav-item">
            <a class="nav-link collapsed" href="sheetsearch2">
                <i class="bi bi-receipt"></i>
                <span>ລາຍ​ລະ​ອຽດລົງ​ຄະ​ແນນ(ຮອງປະ​ທານ)</span>
            </a>
        </li>

        <li class="nav-item">
            <a class="nav-link collapsed" href="deletesheet">
                <i class="bi bi-receipt"></i>
                <span>​​ລົບ​ໃບ​ລົງ​ຄະ​ແນນ​ຜິດ</span>
            </a>
        </li>

        <li class="nav-item">
            <a class="nav-link collapsed" href="oscore">
                <i class="bi bi-sliders"></i>
                <span>​ຜົນ​ຄະ​ແນນ(ປະ​ທານ​ກວດກ​າ​ພັກ)</span>
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link collapsed" href="nscore">
                <i class="bi bi-sliders"></i>
                <span>​ຜົນ​ຄະ​ແນນ(ຮອງປະ​ທານ​ກວດກ​າ​ພັກ)</span>
            </a>
        </li>
    </ul>

</aside><!-- End Sidebar-->